...
public class MoveCraft extends Plugin {
    ...
    static final String version = "0.6.1[b7+]";
    ...
    public void initialize() {
        etc.getLoader().addListener(PluginLoader.Hook.BLOCK_CREATED, this.listener, this,
        ...
        etc.getLoader().addListener(PluginLoader.Hook.BLOCK_RIGHTCLICKED, this.listener, this,
                PluginListener.Priority.MEDIUM);

        etc.getLoader().addListener(PluginLoader.Hook.SIGN_CHANGE, this.listener, this, PluginListener.Priority.MEDIUM);
    ...
    class MoveCraftListener extends PluginListener {
        ...
        public void onPlayerMove(Player player, Location from, Location to) {
            ...
            if (craft != null) {
                ...
                if ((craft.isOnBoard) && (!craft.isOnCraft(player, false))) {
                    ...
                    player.sendMessage("§7type /" + craft.name + " remote for remote control");
        ...
        public boolean onBlockCreate(Player player, Block blockPlaced, Block blockClicked, int itemInHand) {
            ...
            if (blockPlaced != null) {
                Craft craft = Craft.getCraft(world, blockPlaced.getX(), blockPlaced.getY(), blockPlaced.getZ());

                if (craft != null) {
                    if ((blockPlaced.getType() == 321) || (blockPlaced.getType() == 323)
                            || (blockPlaced.getType() == 324) || (blockPlaced.getType() == 330)) {
                        player.sendMessage("§eplease release the " + craft.type.name + " to add this item");
                        return true;
                    }
            ...
            if ((blockClicked.getType() == 63) || (blockClicked.getType() == 68)) {
                if (playerCraft == null) {
                    Sign sign = (Sign) world.getComplexBlock(blockClicked.getX(), blockClicked.getY(),
                            blockClicked.getZ());
                    ...
                    if (craftType != null) {
                        ...
                        String[] groups = (sign.getText(2) + " " + sign.getText(3)).replace(",", "").replace(";", "")
                                .split("[ ]");
        ...
        public void onArmSwing(Player player) {
            ...
            if (craft != null) {
                ...
                if ((!craft.haveControl) || (item == 256) || (item == 257) || (item == 258) || (item == 269)
                        || (item == 270) || (item == 271) || (item == 273) || (item == 274) || (item == 275)
                        || (item == 277) || (item == 278) || (item == 279) || (item == 284) || (item == 285)
                        || (item == 286) || ((item >= 290) && (item <= 294)) || (item == 336)) {
                    return;
                }
                ...
                craft.setSpeed(craft.speed - (int) ((System.currentTimeMillis() - craft.lastMove) / 500L));
                ...
                if ((craft.type.canFly) || (craft.type.canDive)) {
                    ...
                    dy = -(Math.abs(player.getPitch()) >= 25.0F ? 1 : 0) * (int) Math.signum(p);
                ...
                if ((craft.type.canDive) && (!craft.type.canFly) && (craft.waterLevel <= 0) && (dy > 0)) {
                    dy = 0;
                }
                ...
                while (!craft.canMove(dx, dy, dz)) {
                    if (craft.speed == 1) {
                        ...
                        player.sendMessage("§cthe " + craft.name + " won't go any further");
                        return;
                    }
        ...
        private void createCraft(Player player, CraftType craftType, int x, int y, int z, String name) {
            Craft craft = Craft.getCraft(player);
        ...
        private boolean processCommand(CraftType craftType, Player player, String[] split) {
            Craft craft = Craft.getCraft(player);

            if (split.length >= 2) {
                if (split[1].equalsIgnoreCase(craftType.driveCommand)) {
                    createCraft(player, craftType, (int) Math.floor(player.getX()),
                            (int) Math.floor(player.getY() - 1.0D), (int) Math.floor(player.getZ()), null);

                    return true;
                }
                if (split[1].equalsIgnoreCase("setspeed")) {
                    if (craft == null) {
                        player.sendMessage("§eYou don't have any " + craftType.name);
                        return true;
                    }
                    ...
                    if ((speed < 1) || (speed > craftType.maxSpeed)) {
                        player.sendMessage("§cAllowed speed between 1 and " + craftType.maxSpeed);
                        return true;
                    }

                    craft.setSpeed(speed);
                    player.sendMessage("§e" + craft.name + "'s speed set to " + craft.speed);
                ...
                if (split[1].equalsIgnoreCase("setname")) {
                    ...
                    craft.name = split[2];
                    player.sendMessage("§e" + craft.name + "'s name set to " + craft.name);
                    return true;
                }
                if (split[1].equalsIgnoreCase("size")) {
                    ...
                    player.sendMessage("§eThe " + craft.name + " is built with " + craft.blockCount + " blocks");
                    return true;
                }
                if (split[1].equalsIgnoreCase("remote")) {
                    ...
                    if (craft.isOnCraft(player, true)) {
                        player.sendMessage("§eYou are on the " + craftType.name + ", remote control not possible");
                    } else {
                ...
                if (split[1].equalsIgnoreCase("info")) {
                    ...
                    if (craftType.canFly) {
                        player.sendMessage("§ethe " + craftType.name + " can fly");
                    }
                    if (craftType.canNavigate) {
                        player.sendMessage("§ethe " + craftType.name + " can navigate on both water and lava");
                    }
                    if (craftType.canDive) {
                        player.sendMessage("§ethe " + craftType.name + " can dive");
                    }
                    if (craftType.flyBlockType != 0) {
                        player.sendMessage("§crequirement : " + craftType.flyBlockPercent + "%" + " of "
            ...
            player.sendMessage("§e/" + craftType.name + " " + craftType.driveCommand + " : §f" + " "
                    + craftType.driveCommand + " the " + craftType.name);
            player.sendMessage("§e/" + craftType.name + " " + "release : §frelease the " + craftType.name);
            player.sendMessage("§e/" + craftType.name + " " + "remote : §fremote control of the " + craftType.name);
            player.sendMessage("§e/" + craftType.name + " " + "size : §fthe size of the " + craftType.name
                    + " in block");
            player.sendMessage("§e/" + craftType.name + " " + "setname : §fset the " + craftType.name + "'s name");
            player.sendMessage("§e/" + craftType.name + " " + "info : §fdisplays informations about the "
                    + craftType.name);
        ...
        public boolean onCommand(Player player, String[] split) {
            ...
            if ((split[0].equalsIgnoreCase("/movecraft")) && (player.canUseCommand("/movecraft"))) {
                if (split.length >= 2) {
                    if (split[1].equalsIgnoreCase("types")) {
                        for (CraftType craftType : CraftType.craftTypes) {
                            if (craftType.canUse(player).booleanValue()) {
                                player.sendMessage("§e " + craftType.name + " :§f " + craftType.minBlocks + "-"
                    ...
                    if (split[1].equalsIgnoreCase("list")) {
                        ...
                        for (Craft craft : Craft.craftList) {
                            player.sendMessage("§e" + craft.name + " controlled by " + craft.player.getName() + " : "
